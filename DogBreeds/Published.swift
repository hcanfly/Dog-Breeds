...
//  Published
//
//  Created by Gary Hanson on 9/2/21.
//  From: https://www.swiftbysundell.com/articles/published-properties-in-swift/
//
...
struct Published<Value> {
    ...
    private var observations = MutableReference(value: List<(Value) -> Void>()
    )
...
final class MutableReference<Value>: Reference<Value> {
...
extension Published {
    func observe(with closure: @escaping (Value) -> Void) -> Cancellable {
        // To further mimic Combine's behaviors, we'll call
...
// We need to store the closures in such a way that they can be referenced for deletion
// List: from https://www.swiftbysundell.com/articles/picking-the-right-data-structure-in-swift/
